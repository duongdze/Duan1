document.addEventListener('DOMContentLoaded', function() {
    ...
    var descriptionEditor = initQuillEditor('#editor-description', 'input-description');
    ...
    function setupDynamicSection(config) {
        ...
        function appendNewItem(values) {
            ...
            var item = clone.querySelector('.' + config.itemClass) || clone.children[0];
        ...
        if (existingItems.length) {
            ...
            var initialData = [];
            try {
                initialData = listEl.dataset.initial ? JSON.parse(listEl.dataset.initial) : [];
            } catch (error) {
                initialData = [];
            }
        ...
        galleryInput.addEventListener('change', function(e) {
            ...
            files.slice(0, 10).forEach(function(file) {
                ...
                reader.onload = function(evt) {
                    ...
                    col.className = 'col-4';
                    col.innerHTML = '<div class="ratio ratio-4x3 rounded overflow-hidden border"><img src="' + evt.target.result + '" class="w-100 h-100 object-fit-cover" alt=""></div>';
